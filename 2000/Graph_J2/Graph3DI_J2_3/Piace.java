 ...
 public class Piace extends Panel implements MouseListener, MouseMotionListener, KeyListener { 
   ...
   int  tab, x0, y0, kind, step, recINT;           //tab-для навигации по элементам мира;kind-вид элемента;step-перемещение элемента по осям.(x0,y0)-тень последней записанная точка.
   ...
   public void mousePressed(MouseEvent e) 
    {
      ...
      if(e.isShiftDown())
       ...
       {
         int current = numberPoint(x, y);
         if(current < points3D.size())
           next_offset(current);
       }
      else
       {
         ...
         else  Move.mouseDown(this,x,y);  //режим перемещения.  
       }
    }
    ...
    { 
      ...
      if(e.isShiftDown())
      ...
      else
       {
         ...
         else Move.mouseDrag(this,x,y);  //режим перемещения.
       }
    }
    ...
    { 
      ...
      if(e.isShiftDown())
      ...
      else
       {
         ...
         else Move.mouseUp(this);        //режим перемещения.
       }
    }
   ...
   private void SavePoint3D(int i, int x, int y)  //запись новой точки Элемента, "i" - номер точки
    {
...
//////////////////////////////////////////////////Блок навигации по элементам:
   ...
   void next_offset(int t)  ////смена оси вращения 
     {
...
//////////////////////////////////////////////////////Блок реконструкции элементов:
   void reconstruct(Element elem, String axis)   //формирует фигуры вращения, axis-ось вращения.
     { 
       ...
       if(turnSum == 0)
         recINT = k;       //переменная класса
       matrix.identity();
       matrix.rotate(turn, axis, true);
       for (int i=0, t=0, n=0, z=0; i<k; i++, n%=recINT) 
         {        
           ...
           if(n!=0)
            {
             figure.elements.addElement(copy = new Side(this, Element.Poly, select.color, true, false));
             copy.add(z-recINT-1);
             copy.add(z-recINT);
             copy.add(z);
             copy.add(z-1);
...
///////////////////////////////////////////////////////Блок вращения элементов:
   void rotate(String axis, Element e, float rotate) //вращение Element-a "e" вокруг оси "axis"
     {                                                      //на угол "rotate"
       matrix.identity();
        ...
        for (int k=0; k<e.elements.size(); k++)
         {
           ...
           rotate(axis, elem, rotate);
         }  
     } 
///////////////////////////////////////////////////////Блок перемещения элементов:
   void translate(Element e, int dx, int dy, int dz)  //перемещение "e" на расстояние "dx", "dy", "dz"
     {
       matrix.identity();
...
///////////////////////////////////////////////////////Блок масштабирования элементов:
   void scale( Element e, float x, float y, float z) //масштабирование "е" по осям X, Y, Z
     {
       matrix.identity();
       matrix.scale(x, y, z);
       transform(matrix, e.field);
