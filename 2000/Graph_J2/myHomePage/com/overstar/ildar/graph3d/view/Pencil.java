   ...
   static void mouseDown(Piace piace, int x, int y) 
	{
		...
		if(piace.select.getField().size() == 0)   //запись только первой точки элемента-select
		  piace.savePoint3D(piace.numberPoint(piace.figure.getField(), x, y) , x, y);                
 ...
 */
	...
	{ 
	   piace.rect.translate(x - piace.rect.x - 4, y - piace.rect.y - 4);  //"rect" следует за нажатием мыши          
	   if ( (piace.select.isFree())&&(!(piace.rect.contains(piace.x0, piace.y0))) )
		  piace.savePoint3D(piace.numberPoint(piace.figure.getField(), x, y) , x, y);                  
