  ...
  public void actionPerformed(ActionEvent evt)
	{
	  ...
	  if (evt.getSource() instanceof MenuItem)
		...
		{ 
		 if ((obj.equals("Form('A')"))&&(piace.turnSum < (float)(1.5*Math.PI))&&(piace.select.getField().size() != 0))
		   {piace.reconstruct(piace.select, "Z");}
		 if ((obj.equals("Form('B')"))&&(piace.turnSum < (float)(1.5*Math.PI))&&(piace.select.getField().size() != 0))
		   {piace.reconstruct(piace.select, "Y");}
		 if ((obj.equals("Form('D')"))&&(piace.turnSum < (float)(1.5*Math.PI))&&(piace.select.getField().size() != 0))
  ...
  public void paint(Graphics g)   //прорисовка скелета выбранной фигуры.
	{
	  ...
	  if(piace.select.getField().size() != 0)
	   {
		g.setColor(piace.select.getColor());     //прорисовка заполненной стороны-select.
		g.fillPolygon(piace.select.getShade(zero, viewer));
		for (int i=0; i<piace.figure.getSwarms().size(); i++)
		 {
		  Side side = (Side)piace.figure.getSwarms().elementAt(i);
