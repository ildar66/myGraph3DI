   ...
   public void keyTyped(KeyEvent e)
     {
      ...
      else if ((key == 'A')&&(turnSum < (float)Math.PI)&&(select.field.size() != 0))
         {reconstruct(select, "Z");}
      else if ((key == 'B')&&(turnSum < (float)Math.PI)&&(select.field.size() != 0))
         {reconstruct(select, "Y");}
      else if ((key == 'D')&&(turnSum < (float)Math.PI)&&(select.field.size() != 0))
     ...
     { 
       ...
       for (int i=0, t=0; i<k; i++) 
         {        
           ...
           rect.translate(Math.round(p.x) - rect.x - 5, Math.round(p.y) - rect.y - 5); x0 = Math.round(p.x)+offset.x; y0 = Math.round(p.y)+offset.y; //new
         }
...
///////////////////////////////////////////////////////Блок масштабирования элементов:
   public void scale( Element e, float x, float y, float z) //массштабирование "е" по осям X, Y, Z
     {
       matrix.identity();
       matrix.scale(x, y, z);
       transform(matrix, e.field);
       if((e instanceof Side)&&(!(e instanceof Figure))) //массштабирование элементов стороны "е" по осям X, Y, Z
        for (int k=0; k<e.elements.size(); k++)
         {
           Element elem =(Element)e.elements.elementAt(k);
           scale(elem, x, y, z);
     ...
     if (select.field.size() != 0)
       {
        ...
        Point3D p = (Point3D)points3D.elementAt(t); //добавить слежение за "rect"
