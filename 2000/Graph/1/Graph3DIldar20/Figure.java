 ...
 class Side  {                 //элемент фигуры(figure).
 
  Figure figure;               //фигура, которой принадлежит элемент(side).
  Vector field = new Vector(); //набор номеров(number) трехмерных точек из которых состоит элемент.
  boolean full, normal_Left;   //показатели(full) заполненности элемента, нормали(normal_Left). 
  ...
  public void add(int number) //добавить трехмерную точку с номером(number) к элементу(side).
    {
      field.addElement(new Integer(number));
    } 

  public Vector3D normal()    //вычисление вектора нормали к элементу(side).
    { 
...
/////////////////////////////////////////////////////////////////////////////////////////
 public class Figure extends Applet implements MouseListener, MouseMotionListener,                                                                      KeyListener  {   
   ...
   public void SavePoint3D(int i, boolean b, int x, int y)
    {
      if(select.field.size() == 0)
        {sides.addElement(select); tab = sides.size();}      
      select.add(i);
      del.addElement(new Boolean(b));
      if(b)
        points3D.addElement(current = new Point3D(x - offset.x, y - offset.y, 0));             
    }
   ...
   public void mousePressed(MouseEvent e) 
    {
     ...
     for (int i = 0; i < points3D.size(); i++)
      {
       ...
       if (r.contains(x0, y0)) 
         {  
           if(!select.field.contains(new Integer(i))&&(!tab_flag)&&(select.field.size() == 0)) 
             SavePoint3D(i, false, x, y);            
           move = true;        
           Xstart = x; Ystart = y;   
     ...
     if ((tab_flag)||(select.field.size() != 0))
       return ;
     SavePoint3D(points3D.size(), true, x, y);
     Xstart = x; Ystart = y;
   ...
   public void mouseReleased(MouseEvent e) 
    { 
           ...
           for (int i = 0; i < points3D.size(); i++)
            {
             ...
             if (r.contains(x0, y0)) 
               {  
                 ...
                 System.out.println("sel.field.size:up_old"+select.field.size()); 
                 return ;            
               }
            }       
           if (tab_flag)
      ...
      repaint(rec.x-5, rec.y-5, rec.width+10, rec.height+10);       
      System.out.print(points3D.size()+";"+sides.size()+";");
      System.out.println("sel.field.size:up_new"+select.field.size());       
    }
          ...
          if (select.field.size() == 0)
            {
              ...
              tab = sides.size();
              if (points3D.size() != 0)
                {select = (Side)sides.lastElement(); FP.setControl();}
              else
                offset = null;              
            }                     
          repaint();      
