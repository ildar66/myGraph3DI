 ...
 class Side  {
  ...
  public void getShade(Point offset, String view)
    {
      if(field.size() == 0)  
         return;
      X = new int[field.size()];
      Y = new int[field.size()];    
      int xp, yp, zp, t;
      for (int i=0; i<field.size(); i++) 
        {
          ...
          if ((view.equals("view Face"))||(view.equals("shade")))    
            {X[i] = offset.x + xp; Y[i] = offset.y + yp;}
          else if (view.equals("view Right"))
            {X[i] = offset.x + zp; Y[i] = offset.y + yp;}
          else if (view.equals("view Up"))
            {X[i] = offset.x + xp; Y[i] = offset.y - zp;}      
        }
    }
  ...
  public void draw(Graphics g, Point offset, String view)
    {      
      g.setColor(color);
      getShade(offset, view);
      if ((full)&&((kind == Poly)||(kind == freePoly)))      
        g.fillPolygon(X, Y, X.length);
      else if ((!full)&&((kind == Poly)||(kind == freePoly))) 
        g.drawPolygon(X, Y, X.length); 
      else if ((kind == Line)||(kind == freeLine)) 
