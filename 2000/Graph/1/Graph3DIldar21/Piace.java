 ...
 class Element  {              //элемент стороны фигуры(figure).
  ...
  public float weight()
    {
      ...
      for (int i=0; i<field.size(); i++) 
        {
          t = ((Integer)field.elementAt(i)).intValue();
          sumZ += ((Point3D)piace.points3D.elementAt(t)).z; 
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////
 public class Piace extends Applet implements MouseListener, MouseMotionListener, KeyListener {   
  ...
  // int tab_add = -1;
      ...
      if(select.field.size() == 0)
        { 
          if(select instanceof Side)
            ...
            SavePoint3D(select.field.size(), x, y);
        }
      Xstart = x; Ystart = y;                   //?
   ...
   public void message()
     {System.out.println("точек:"+points3D.size()+";сторон:"+figure.sides.size()+";select:"+select.field.size());}
 ...
 /*     else if ((key == 'c')&&(select.field.size() > 1)) 
          ...
          if (select.field.size() == 0)
            {
              ...
              tab = figure.sides.size();
              if (points3D.size() != 0)
                {select = (Side)figure.sides.lastElement(); FP.setControl();}
              else
                offset = null;              
            }                     
          repaint();      
   ...
   public void paint(Graphics g)
     { 
       ...
       select.draw(g, offset, "view Face");
