 ...
 public class Piace extends Applet implements MouseListener, MouseMotionListener, KeyListener {   
      ...
      if ((key == 'n')&&(select.field.size() != 0))
        {
          ...
          translateX = ((Point3D)points3D.elementAt(t)).x;//Point3D (x,y,z)???
      ...
      for (int i=0, t=0, n=0; i<vector.size(); i++)
       {
        ...
        if((select instanceof Side)&&((n = Ncopy(hash, t)) != -1)) //если существующая точка стороны нового эл-та ,то:
          select.add(n);
        else
         {
          n = points3D.size();
   ...
   public int Ncopy(Hashtable hash,int t)
     {
       if(hash.containsKey(new Integer(t)))
         return ((Integer)hash.get(new Integer(t))).intValue();
       else
         return -1; 
     } 
   ...
   public void paint(Graphics g)
     { 
       ...
       if (offset != null)
         {g.setColor(Color.black);g.fillOval(offset.x - 4, offset.y - 4, 8, 8);} 
