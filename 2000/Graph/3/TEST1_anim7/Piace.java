...
///////////////////////////////////////////////////////Блок перемещения элементов:
   synchronized void rotate(Figure f)     //new : вращение Element-a "e" 
     {                                                           
      matrix.identity();
      if(f.numberZero != -1)
       {
        Point3D p = (Point3D)points3D.elementAt(f.numberZero);
        matrix.translate(-p.x, -p.y, -p.z);                      //изменение координат мира(points3D)
        matrix.rotate(f.animX, "X", false);               
        matrix.rotate(f.animY, "Y", false);               
        matrix.translate(p.x, p.y, p.z);                         //изменение координат мира(points3D)
       }
      else
       {
