 ...
 */
 ...
 public class Swarm {
   ...
   protected Axis axis; 			          //ось вращения
 ...
 */
   synchronized protected void rotate(Matrix3D matrix, Axis ax, float rotate)  //new : вращение Element-a "e" вокруг оси(a,b)
	 {                                                                 //на угол=rotate
	  if ( (axis != null) && (axis.a != -1) && (axis.b != -1) )
	   {
		float animX, animY, animZ;
		matrix.identity();
		Point3D pA = (Point3D)piace.points3D.get(ax.a);
		Point3D pB = (Point3D)piace.points3D.get(ax.b);
 ...
 */
 protected void setA(int a) {axis.a = a;}             
/**
 * Insert the method's description here.
 ...
 */
 protected void setB(int b) {axis.b = b;}       
/**
 * Insert the method's description here.
...
protected float weight() //используется для сортировки очереди рисования сторон.
{
	...
	return ( (field.size() == 0) ? 0f : (sumZ / field.size()) );
}
}  
