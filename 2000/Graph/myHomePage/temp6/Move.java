 ...
 import java.awt.*;import java.util.*; public class Move {   //движение элементов с помощью мыши.
  ...
  static void mouseDown(Piace piace, int x, int y, MouseEvent e)
	{ 
	  ...
	  if(e.isControlDown())
			 isControlDown=true;    //нажата ли клавиша "Ctrl"(настройка)
	   ...
	   ytheta = (x0 - x) * ( 360.0f / piace.getSize().height );
		   if(isControlDown) {   //настройка 
			   if(choice==1) {
				  ...
				  piace.figure.animXmax += Math.abs(xtheta);//перенесли в обычный
 ...
 * Creation date: (22.10.2001 16:39:25)
 */
 static void mouseClicked(Piace piace, int x, int y) {

	for(ListIterator list = Piace.figures.listIterator(); list.hasNext();) {
	    Figure figure= (Figure)list.next();	
	    for(ListIterator list2 = figure.elements.listIterator(); list2.hasNext();) {
	      Side side= (Side)list2.next();	  
