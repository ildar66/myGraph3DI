	 ...
	 for (int i=0, t=0; i<field.size(); i++) 
	  {
		...
		Point3D p = (Point3D)piace.points3D.elementAt(t);
 ...
 */
  protected Vector3D normal(boolean normal_Left, int n1, int n2, int n3) //вычисление вектора нормали к трем точкам
	{                                                                    //по показателю нормали "normal_Left".
	  int a = ((Integer)field.elementAt(n1)).intValue(); //номер n1 точки в piace.
	  int b = ((Integer)field.elementAt(n2)).intValue(); //№ n2 точки в piace.
	  int c = ((Integer)field.elementAt(n3)).intValue(); //№ n3 точки в piace.
  ...
  protected synchronized void transform(Matrix3D matrix) //метод, вызываемый после изменения СК
	 {
	   for (int i=0, t=0; i<field.size(); i++)      
		{
		  t = ((Integer)field.elementAt(i)).intValue();
		  matrix.transform((Point3D)piace.points3D.elementAt(t));
		}
	 }
/**
 * Insert the method's description here.
