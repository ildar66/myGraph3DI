 ...
 import java.awt.*;
   ...
   ArrayList points3D = new ArrayList();                 //Вектор физических точек(3D).
   ArrayList figures = new ArrayList();                  //Вектор всех фигур.
   ...
   public Piace(Graph3DI graph)
	{
	  ...
	  figure.elements.add(select = new Side(this, kind, color, full, false));
	  ...
	  for (int i=0; i<figures.size(); i++)  //"i" - текущий номер фигуры
	   {
		 ...
		 for (int k=0; k<figure.elements.size(); k++)    //"k"- текущая сторона в "i"-ой фигуре 
		  {
		   Side side = (Side)figure.elements.get(k);
		   for (int m=0, n=0; m<side.field.size(); m++)  //"m"- текущая 3Dточка в "k"-ой стороне
			{
			  n = ((Integer)side.field.get(m)).intValue();//"n"- номер "m"-ой 3Dточки
			  if(n==t)                                          
   ...
   void newElement() //создание нового элемента стороны-select
	 {
	   if(select instanceof Side)  //сохранение стороны-select в "copy"
		 copy = (Side)select;        
	   copy.elements.add(select = new Element(this, kind, color, full));
//	   message();
	 }
	  ...
	  for (int i=0; i<figures.size(); i++)
	   {
		...
		if(figure.net) {            //прорисовка пустыми овалами точек сторон фигуры
		   ...
		   for(int k=0; k<figure.elements.size(); k++)
			 ((Side)figure.elements.get(k)).net(g);
		}
	   }       
...
/////////////////////////////////////////////////////Блок отмены методов "new ..." 
   void copy(Swarm e) //копирование элемента "е"
	 ...
	 }   private void copyElement(Swarm e, Hashtable hash)  //рекурсивное копирование элемента "е"
	 {
	  if(e instanceof Figure) //копирование фигуры 
	   ...
	   { 
		figure.elements.add(select = new Side(this, ((Side)e).kind, ((Side)e).color, ((Side)e).full, ((Side)e).normal_Left));
	   ...
	   { 
		copy.elements.add(select = new Element(this, ((Element)e).kind, ((Element)e).color, ((Element)e).full));
		copyPoint3D(e.field, hash);
	   }         
	 }   private void copyPoint3D(ArrayList vector, Hashtable hash) //копирование 3Dточек "vector"
	 {
	  for (int i=0, t=0, n=0; i<vector.size(); i++)
	   {
		t = ((Integer)vector.get(i)).intValue();
	   ...
	   if(e.field.size() != 0)
		{
		 ...
		 e.field.remove(e.field.size()-1);      //new //old : e.field.removeElement((Integer)e.field.lastElement());
   ...
   synchronized void rotate(String axis, Swarm e, float rotate) //вращение Element-a "e" вокруг оси координат"axis"
	 {                                                      //на угол "rotate"
	   ...
	   if(e instanceof Side) // учет 3Dточек элементов "elem" стороны "Side"
		for (int k=0; k<((Side)e).elements.size(); k++)
		 {
		   Element elem =(Element)((Side)e).elements.get(k);
