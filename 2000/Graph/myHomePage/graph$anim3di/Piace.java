   ...
   public void keyTyped(KeyEvent e)
	 {
	  ...
	  if ((key == 'e')&&(select.field.size() != 0))
		 ...
		 { matrix.identity(); matrix.rotate(rotate,"Z", false);for (int i=0; i<points3D.size(); i++) matrix.transform((Point3D)points3D.elementAt(i));}
             ...
             rotate(figure, figure.a, figure.b, rotate);  //new
      ...
      if ((a!=b) && (Math.max(a,b)< points3D.size())) 
       {
        ...
        mag = axis.mag();
        animX = axis.x* rotate/mag ; 
        animY = axis.y* rotate/mag ; 
        animZ = axis.z* rotate/mag ; 
...
///////////////////////////////////////////////////////Блок для рисования элементов:
   ...
   synchronized private void sort(Vector v)                  //сортировка "vector"-a по удаленности его элементов 
	 {
	   int j = v.size();
	   while(--j > 0)
		{ 
